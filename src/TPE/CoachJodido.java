...
public class CoachJodido extends Coach{
    ...
    public void teamAdd(Participant p) {
        /*los ints hacen la cuenta de cuántos langs,insts y gens
        tiene el participante que coincidan con los que pide pref*/
        int prefLang = 0;
        int prefInst = 0;
        int prefGen = 0;
        ...
        if (prefLang==pref.getLangs().size()&&prefInst==pref.getInsts().size()&&prefGen==pref.getGens().size())
            super.teamAdd(p);
    }
}
